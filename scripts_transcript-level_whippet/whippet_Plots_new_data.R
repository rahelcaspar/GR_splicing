library(ggplot2)
#visualize whippet results - new data
...
## read new_data samples-table: 
dataset_samples <- read.csv("MA/Samples_Tabellen/new_data_Samples_Uebersicht.csv")
dataset_samples[,"sample"] <- substring(dataset_samples[,"sample"],2)
...
for(sampleX in dataset_samples[,"sample"]){
  print(sampleX)
  file_sampleX <- read.csv(gzfile(paste0("/nfs/proj/gr_splicing/whippet-files/output_quant/new_data/",sampleX,".psi.gz")), sep = "\t")
  ...
  condX <- dataset_samples[dataset_samples$sample==sampleX,"condition"]
  for(ASx in AS_types){
...
#grouped barplot 
png("/nfs/proj/gr_splicing/Ergebnisse_new_data/whippet-plots/new_data_AS_types_barplot.png", width = 700)
ggplot(data, aes(fill=condition, y=value, x=AS_Type)) + 
  geom_bar(position="dodge", stat="identity") + 
  labs(title = "Frequency of AS Types in Hepatocytes", x = "Alternative Splicing Type", y = "Frecuency") + 
  labs(fill = "Condition") +
...
#grouped violin plot
png("/nfs/proj/gr_splicing/Ergebnisse_new_data/whippet-plots/new_data_AS_types_violinplot.png", width = 1500)
ggplot(psi_data, aes(fill=psi_condition, y=psi, x=psi_AS_Type)) + 
  geom_violin(position="dodge") +
  labs(title = "Distribution of PSI Values in Hepatocytes", x = "Alternative Splicing Type", y = "PSI") + 
  labs(fill = "Condition") +
  theme(plot.title = element_text(hjust = 0.5))
dev.off()
