library(ggplot2)
#visualize whippet results - public data - ko
...
## read public_knockout samples-table: 
dataset_samples <- read.csv("MA/Samples_Tabellen/samples_knockout.csv")

#get the AS information from the psi.gz files
sample_events <- list()
cond_stats <- list(KO = c(), WT = c())
cond_stats[["KO"]] <- list(CE = c(), AA = c(), AD = c(), RI = c(), TS = c(), TE = c(), AF = c(), AL = c())
cond_stats[["WT"]] <- list(CE = c(), AA = c(), AD = c(), RI = c(), TS = c(), TE = c(), AF = c(), AL = c())
for(sampleX in dataset_samples[,"sample"]){
  print(sampleX)
  file_sampleX <- read.csv(gzfile(paste0("/nfs/proj/gr_splicing/whippet-files/output_quant/public_ko_wt/",sampleX,".psi.gz")), sep = "\t")
  ...
  condX <- dataset_samples[dataset_samples$sample==sampleX,"knockout"]
  for(ASx in AS_types){
...
#grouped barplot 
png("/nfs/proj/gr_splicing/Ergebnisse_public_data/whippet-plots/public_ko_AS_types_barplot.png", width = 700)
...
#grouped violin plot
png("/nfs/proj/gr_splicing/Ergebnisse_public_data/whippet-plots/public_ko_AS_types_violinplot.png", width = 1500)
