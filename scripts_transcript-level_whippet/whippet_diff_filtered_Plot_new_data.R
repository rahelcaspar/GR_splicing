...
#load output data from whippet delta (hepatocytes dex)
file_diff <- read.table(gzfile("/nfs/proj/gr_splicing/whippet-files/output_delta/whippet-delta_new_data.diff.gz"), header = TRUE)

#filter whippet-delta output (hepatocytes dex) for |DeltaPsi| > 0.1 and Probability > 0.9
file_diff <- file_diff[(abs(file_diff$DeltaPsi) > 0.1),]
file_diff <- file_diff[(file_diff$Probability > 0.9),]

#save in file
write.table(file_diff, file = "/nfs/proj/gr_splicing/whippet-files/output_delta/whippet-delta-filtered_new_data.tsv", quote = FALSE, sep = "\t", row.names = FALSE)
...
#grouped barplot 
png("/nfs/proj/gr_splicing/Ergebnisse_new_data/whippet-plots/new_data_diff_AS_types_barplot.png", width = 700)
